...
class FilmSpiderSpider(scrapy.Spider):
    name = "film_spider"
    allowed_domains = ["ru.wikipedia.org", "ru.m.wikipedia.org", 'www.imdb.com']
    ...
    def get_list_films(self, response):
        ...
        next_page = response.css('div[id=mw-pages]').css('a::attr(href)').getall()
        if next_page:
    ...
    def parse_film(self, response):
        ...
        if response.css('th.infobox-above::text').get():
            ...
            name = response.css('table.infobox.infobox-a323cc30500039ad').css('b::text').get()
        
        # Parse genre of film
        if response.css('span[data-wikidata-property-id="P136"]').css('a::text').getall():
            genre = response.css('span[data-wikidata-property-id="P136"]').css('a::text').getall()
        else:
            genre = response.css('span[data-wikidata-property-id="P136"]').css('span::text').getall()
        
        # Parse director of film
        if response.css('span[data-wikidata-property-id="P57"]').css('span::text').getall():
            director = response.css('span[data-wikidata-property-id="P57"]').css('span::text').getall()
        else:
            director = response.css('span[data-wikidata-property-id="P57"]').css('a::text').getall()

        # Parse country of film
        if response.css('span[data-wikidata-property-id="P495"]').css('span.wrap::text').getall():
            country = response.css('span[data-wikidata-property-id="P495"]').css('span.wrap::text').getall()
        else:
            country = response.css('span[data-wikidata-property-id="P495"]').css('a::text').getall()
